...
async def filiere_departement_data_by_id_dep(id:int):
    # Créer une session
    Session = sessionmaker(bind=con)
    session = Session()
...
async def departement_id(nom:str):
    ...
    # Effectuer la requête pour récupérer les filières avec leurs départements
    departements = session.query(Departements).filter(Departements.nom == nom).first()
...
@filiere_router.get("filiere_by_id/{id}")
async def read_data_by_id(id:int):
    ...
    # Effectuer la requête pour récupérer les filières avec leurs départements
    filieres = session.query(Filieres).join(Filieres.departements).filter(Filieres.id==id)
